...
class LocationManager(private var currentLocationListener: CurrentLocationListener, var mContext: Context, requireLastLocation: Boolean = true) : Listener, LocationData.AddressCallBack {
    ...
    private fun getLocationPermission() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            ...
            val foreground = ActivityCompat.checkSelfPermission(mContext, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED
    ...
    private fun goToLocationSetting() {
        val cleverDialog = IADialog(mContext as Activity, "", mContext.getString(R.string.instructions_to_turn_on_location_permission), mContext.getString(R.string.settings), mContext.getString(R.string.not_now), false)
        cleverDialog.setOnItemClickListener(object : IADialog.OnClickListener {
            ...
            override fun onPositiveClick() {
                ...
                (mContext as Activity).startActivityForResult(settingsIntent, AppRequestCode.LOCATION_SETTING_REQUEST)
            }
