...
class CryptLib {
    ...
    private fun encryptDecrypt(inputText: String, encryptionKey: String, mode: EncryptMode, initVector: String): ByteArray {
    ...
    fun encryptPlainTextWithRandomIV(plainText: String, key: String): String {
        val bytes = encryptDecrypt(generateRandomIV16() + plainText, SHA256(key, 32), EncryptMode.ENCRYPT, generateRandomIV16())
        return Base64.encodeToString(bytes, Base64.DEFAULT)
    }

    @Throws(Exception::class)
    fun decryptCipherTextWithRandomIV(cipherText: String, key: String): String {
        val bytes = encryptDecrypt(cipherText, SHA256(key, 32), EncryptMode.DECRYPT, generateRandomIV16())
