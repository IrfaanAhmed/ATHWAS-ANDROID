...
class OfferProductListViewModel(private val baseRepository: BaseRepository) : BaseViewModel(),
    LifecycleObserver {
    ...
    var productList = MutableLiveData<MutableList<OffersResponse.Product>>()
    ...
    fun setVariable(mBinding: ActivityOfferProductListBinding, intent: Intent) {
    ...
    * */
    private fun getPopularOrDiscountedProductListing(params: HashMap<String, String>) = liveData(
        Dispatchers.Main) {
        emit(Resource.loading(data = null))
        try {
            if (type == 1) {
    ...
    * */
    private fun getDealOfTheDayBannerProductListing(params: HashMap<String, String>, banner_id: String) = liveData(
        Dispatchers.Main) {
        emit(Resource.loading(data = null))
        try {
